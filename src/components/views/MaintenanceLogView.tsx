...
export const MaintenanceLogView: React.FC<MaintenanceLogViewProps> = ({
  ...
  return (
    <div id="maintenance-log-view" className="space-y-6 pb-8 animate-in fade-in duration-200">
      {/* Header */}
      ...
      <div className="space-y-4">
        ...
        <div className="md:hidden space-y-4">
          {filteredOrders.map((wo) => (
            <div key={wo.id} className="bg-white dark:bg-[#151719] rounded-lg border border-slate-200 dark:border-[#24272A] p-4 shadow-sm space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  <div className="font-mono font-bold text-slate-900 dark:text-[#E1E4E6]">{wo.id}</div>
                  <div className="font-mono text-xs font-semibold text-slate-500 dark:text-[#8A929B]">{wo.machineId}</div>
                </div>
                <span
                  className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                    wo.priority === 'urgent'
              ...
              <div>
                <div className="text-[10px] font-medium uppercase tracking-wider text-slate-500 dark:text-[#8A929B]">Task / Issue</div>
                <div className="text-sm font-medium text-slate-900 dark:text-[#E1E4E6] mt-0.5">{wo.issue}</div>
              </div>

              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <div className="text-[10px] font-medium uppercase tracking-wider text-slate-500 dark:text-[#8A929B]">Scheduled Date</div>
                  <div className="font-mono text-slate-700 dark:text-[#C1C8CD] mt-0.5">{wo.scheduledDate}</div>
                </div>
                <div>
                  <div className="text-[10px] font-medium uppercase tracking-wider text-slate-500 dark:text-[#8A929B]">Status</div>
                  <div className="mt-0.5">
                    <span
                      className={`px-2 py-0.5 rounded text-[11px] font-medium capitalize inline-block ${
                        wo.status === 'overdue'
                          ? 'text-red-400 bg-red-500/15 border border-red-500/25 font-bold'
                          : wo.status === 'in_progress'
                          ? 'text-amber-400 bg-amber-500/15 border border-amber-500/25 font-semibold'
              ...
              <div className="pt-3 flex justify-between items-center border-t border-slate-100 dark:border-[#24272A]">
                <div className="text-xs font-medium text-slate-600 dark:text-[#A1A8AD] flex items-center gap-1.5">
                ...
                <div>
                  {wo.status !== 'completed' ? (
                    <button
                      onClick={() => onUpdateStatus(wo.id, 'completed')}
                      className="text-xs px-3 py-1.5 rounded bg-slate-100 dark:bg-[#1A1D1F] hover:bg-green-500/15 text-slate-700 dark:text-[#E1E4E6] hover:text-green-400 border border-slate-200 dark:border-[#24272A] font-medium transition-colors"
                    >
                      Complete
                    </button>
                  ) : (
                    <span className="text-xs text-slate-400 dark:text-[#4B5259] font-mono bg-slate-50 dark:bg-[#0F1113] px-2 py-1 rounded border border-slate-100 dark:border-[#24272A]">
                      Signed Off
                    </span>
                  )}
                </div>
